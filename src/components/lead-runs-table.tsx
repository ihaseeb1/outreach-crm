...
/**
 * Lead-run history with selection + delete. Each completed run links to the
 * scraped businesses it queued (Prospecting), where their emails are extracted
 * and the contacts — tagged `client-lead` — can be enrolled in a campaign.
 */
...
export function LeadRunsTable({ runs }: { runs: LeadRun[] }) {
  ...
  async function deleteRuns(body: { ids?: string[]; all?: boolean }) {
    const what = body.all ? "all lead history" : `${body.ids?.length ?? 0} run(s)`;
    if (!window.confirm(`Delete ${what}? The queued businesses and contacts are kept.`)) return;
    setBusy(true);
    setError(null);
    try {
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-3">
        <h2 className="text-sm font-semibold">Lead run history</h2>
        <button
            ...
            <tr>
              <th className="w-8">
                <input
                  type="checkbox"
                  checked={allSelected}
                  onChange={() => setSelected(allSelected ? new Set() : new Set(deletableIds))}
                />
              </th>
              <th>Industry</th>
              <th>Location</th>
            ...
            {runs.length === 0 && (
              <tr>
                <td colSpan={8} className="hint py-6 text-center">
                  No lead runs yet. Start one above.
                </td>
              </tr>
            )}
            {runs.map((run) => (
              <tr key={run.id}>
                ...
                <td className="font-medium">{run.industry}</td>
                <td>{run.location ?? "—"}</td>
                ...
                <td>
                  {run.found_count > 0 ? (
                    <Link
                      ...
                      title="View the queued businesses in Prospecting"
                    >
                      {run.found_count}
                    </Link>
                  ) : (
